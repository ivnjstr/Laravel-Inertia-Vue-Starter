...
<template>
    <div>
        ...
        >
            {{ label }}
            <!--same prop for for attribute also for the name input below-->
        </label>

        <div class="relative mt-1 rounded-md"> <!--wrapper for input field-->
            <div
                class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"
            >
                <span class="grid place-content-center text-sm text-slate-400">
                    <i :class="`fa-solid fa-${icon}`"></i>
                </span> <!--This icon become dynamic by adding another prop we are calling icon-->
            </div>
...
<!--Now import the field to the Register.vue-->